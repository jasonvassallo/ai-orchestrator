...
All UIs (CLI, TUI, GUI, Menu Bar) display real-time processing indicators showing stages: validating, classifying, routing, generating, thinking, searching.

### Conversation Export
Export conversations to markdown or JSON:
- CLI: `-o response.md` or `-o response.json`
- TUI: Ctrl+E
- GUI: Ctrl/Cmd+E or File menu
- Menu Bar: Export Last Query

### Model Attribution
Every response includes an attribution tag `*[Generated by {model_name}]*` so models can identify their own responses vs. other models' responses.

### Incognito Mode
When enabled, new messages are not saved to conversation history, but existing history is preserved and still sent to models for context.

### Conversation Compaction
Summarize long conversations to save context tokens while preserving full history in SQLite:
- TUI: Ctrl+K
- GUI: Ctrl/Cmd+K or File > Compact History
- Menu Bar: Compact History

Use `orchestrator.compact_conversation()` programmatically. Access summaries via `orchestrator.compaction_summaries`.

### Timestamps
`APIResponse` includes `timestamp` field (ISO format). Conversation history messages include `timestamp` for audit trails and security analysis.

### Security Analyzer
Post-incident security analysis runs after every query:
- **Prompt injection detection**: instruction override, jailbreaks, DAN mode, identity manipulation, prompt extraction
- **Response leakage detection**: prompt disclosure, instruction references
- **Dangerous content detection**: code execution patterns, credentials in responses
- **Temporal analysis**: rapid request detection, high request rate warnings

Alerts are logged with severity levels: LOW, MEDIUM, HIGH, CRITICAL. Access programmatically via `orchestrator.get_security_alerts()` and `orchestrator.get_alerts_by_severity(SecuritySeverity.HIGH)`.
