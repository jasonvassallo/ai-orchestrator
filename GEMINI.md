...
- All UIs (CLI, TUI, GUI, Menu Bar) support animated status indicators showing processing stages (validating, classifying, routing, generating, etc.).
- All UIs support conversation export (CLI: `-o` flag, TUI: Ctrl+E, GUI: Ctrl/Cmd+E, Menu Bar: Export Last Query).
- **Model Attribution**: Every response includes an attribution tag `*[Generated by {model_name}]*` so models can identify their own responses vs. other models' responses.
- **Incognito Mode**: When enabled, new messages are not saved to conversation history, but existing history is preserved and still sent to models for context.
- **Conversation Compaction**: All UIs support compacting conversation history (TUI: Ctrl+K, GUI: Ctrl/Cmd+K or File menu, Menu Bar: Compact History). This summarizes the conversation for the model while preserving full history in SQLite for user viewing. Users can access both the summary and full history.
- **Timestamps**: APIResponse includes `timestamp` field (ISO format). Conversation history messages include `timestamp` for audit trails and security analysis.
- **Security Analyzer**: Post-incident security analysis runs after every query. Detects prompt injection attempts (instruction override, jailbreaks, DAN mode), response leakage (prompt disclosure), and dangerous content (code execution patterns). Alerts are logged with severity levels (LOW, MEDIUM, HIGH, CRITICAL). Use `orchestrator.get_security_alerts()` to programmatically access alerts.
